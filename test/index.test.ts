...
describe('fetchDomainInfo', () => {
  ...
  test('should return domain info for example.com', async () => {
    ...
    expect(domainInfo?.sslData.certificate).toContain('-----BEGIN CERTIFICATE-----');
    expect(domainInfo?.sslData.certificate).toContain('-----END CERTIFICATE-----');
    ...
    if (domainInfo?.sslData.intermediateCertificate) {
      expect(domainInfo?.sslData.intermediateCertificate).toContain('-----BEGIN CERTIFICATE-----');
      expect(domainInfo?.sslData.intermediateCertificate).toContain('-----END CERTIFICATE-----');
    }

    expect(domainInfo?.sslData).toHaveProperty('rootCertificate');
    if (domainInfo?.sslData.rootCertificate) {
      expect(domainInfo?.sslData.rootCertificate).toContain('-----BEGIN CERTIFICATE-----');
      expect(domainInfo?.sslData.rootCertificate).toContain('-----END CERTIFICATE-----');
    }
  ...
  test('should throw error for empty domain', async () => {
    await expect(fetchDomainInfo('')).rejects.toThrow('Domain name cannot be empty');
  });

  test('should throw error for invalid domain', async () => {
    await expect(fetchDomainInfo('invalid')).rejects.toThrow('Invalid domain name');
  });
});
