#!/usr/bin/env node
import { fetchDomainInfo, RequestOptions, extractSubdomain, getRootDomain } from '../index';
...
const options: RequestOptions = { debug };
...
async function main(): Promise<void> {
  try {
    ...
    if (subdomain) {
      ...
      console.log(
        chalk.blue(`For subdomain queries, A and CNAME records are specific to the subdomain,`)
      );
      console.log(chalk.blue(`while other DNS records are from the root domain ${rootDomain}`));
    }
    ...
    if (domainInfo.sslData.details) {
      ...
      console.log(
        `  - Valid: ${domainInfo.sslData.valid ? chalk.green('✅ Yes') : chalk.red('❌ No')}`
      );
      console.log(`  - Valid from: ${domainInfo.sslData.details.validFrom.toLocaleDateString()}`);
      console.log(`  - Valid until: ${domainInfo.sslData.details.validTo.toLocaleDateString()}`);
      console.log(
        `  - Days until expiration: ${Math.floor(
          (domainInfo.sslData.details.validTo.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
        )}`
      );
    } else {
      ...
      console.log(`  - Valid from: ${new Date(domainInfo.sslData.validFrom).toLocaleDateString()}`);
    ...
    console.log(`  - Server software: ${domainInfo.serverData || chalk.gray('Not available')}`);
    console.log(
      `  - HTTP Status: ${
    ...
    if (domainInfo.dnsData) {
      ...
      console.log(`  - CNAME: ${domainInfo.dnsData.CNAME || chalk.gray('None')}`);
    ...
    if (domainInfo.whoisData) {
      ...
      console.log(
        `  - Registrar: ${domainInfo.whoisData.registrar || chalk.gray('Not available')}`
      );
      ...
      if (domainInfo.whoisData.registrarIanaId) {
        console.log(`  - Registrar IANA ID: ${domainInfo.whoisData.registrarIanaId}`);
      }
      ...
      if (domainInfo.whoisData.creationDate) {
        console.log(`  - Created: ${domainInfo.whoisData.creationDate.toLocaleDateString()}`);
      }

      if (domainInfo.whoisData.updatedDate) {
        console.log(`  - Last Updated: ${domainInfo.whoisData.updatedDate.toLocaleDateString()}`);
      }

      if (domainInfo.whoisData.expirationDate) {
        const now = new Date();
        const daysUntilExpiration = Math.floor(
          (domainInfo.whoisData.expirationDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
        );
      ...
      if (
        ...
        if (domainInfo.whoisData.registrant.organization) {
          console.log(`  - Organization: ${domainInfo.whoisData.registrant.organization}`);
        }

        if (domainInfo.whoisData.registrant.country) {
          console.log(`  - Country: ${domainInfo.whoisData.registrant.country}`);
        }
      ...
      // Domain status
      if (domainInfo.whoisData.statusCodes && domainInfo.whoisData.statusCodes.length > 0) {
      ...
      if (domainInfo.whoisData.nameServers && domainInfo.whoisData.nameServers.length > 0) {
      ...
      // Show raw WHOIS data in debug mode
      if (debug) {
        console.log('\n[DEBUG] Raw WHOIS Response:');
        console.log('='.repeat(80));
        console.log(domainInfo.whoisData.rawText);
        console.log('='.repeat(80));
      }
    } else {
    ...
    if (debug) {
      // In debug mode, we've already seen the raw data and parsed data
      // Just show a summary here
      console.log('\n[DEBUG] WHOIS Summary:');
    }

    console.log(`\nWHOIS information for ${formatDomain(domain)}:`);
    console.log('='.repeat(80));

    if (domainInfo.whoisData) {
      if (domainInfo.whoisData.registrar) {
        console.log(`Registrar: ${domainInfo.whoisData.registrar}`);
      }
      if (domainInfo.whoisData.registrarUrl) {
        console.log(`Registrar URL: ${domainInfo.whoisData.registrarUrl}`);
      }
      ...
      if (domainInfo.whoisData.creationDate) {
        console.log(`Creation Date: ${domainInfo.whoisData.creationDate.toLocaleDateString()}`);
      }
      if (domainInfo.whoisData.updatedDate) {
        console.log(`Updated Date: ${domainInfo.whoisData.updatedDate.toLocaleDateString()}`);
      }
      if (domainInfo.whoisData.expirationDate) {
        console.log(`Expiration Date: ${domainInfo.whoisData.expirationDate.toLocaleDateString()}`);
      }

      if (domainInfo.whoisData.registrant) {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Invalid domain name')) {
        ...
        error.message.includes('Could not fetch SSL data') ||
        ...
        console.error(chalk.yellow('  - The SSL certificate is invalid or self-signed'));
        console.error(chalk.yellow('  - The domain might be using an outdated SSL protocol'));
        console.error(chalk.yellow('  - Try using a longer timeout with --timeout option'));
        console.error(
          chalk.yellow('  - Try using --debug flag for more detailed error information')
        );
        ...
        console.error(chalk.yellow('  - Your network or DNS resolver is having issues'));
        console.error(
          chalk.yellow(
        ...
        console.error(chalk.yellow('  - A firewall is blocking the connection'));
        console.error(chalk.yellow('  - Try increasing the timeout with --timeout option'));
      } else if (error.message.includes('ENOTFOUND')) {
        console.error(chalk.yellow('\nSuggestion: Domain not found. This could be because:'));
        console.error(chalk.yellow("  - The domain doesn't exist or is misspelled"));
        console.error(chalk.yellow("  - Your DNS resolver can't resolve this domain"));
        ...
        console.error(
          chalk.yellow('  - The server is not accepting connections on port 443 (HTTPS)')
        );
        ...
        console.error(chalk.yellow('  - Verify the domain exists and is accessible'));
        console.error(
          chalk.yellow('  - Try using --debug flag for more detailed error information')
        );
        console.error(chalk.yellow('  - Try increasing the timeout with --timeout option'));
